...
const static CGFloat                    kStackMenuDefaultItemsSpacing                   = 2;
...
- (void)setStackIconClosed:(BOOL)closed {
    UIImageView *icon = [[_baseButton subviews] objectAtIndex:0];
    float angle = closed ? 0 : (M_PI * (_openAngle) / 180.0);
    [UIView animateWithDuration:0.3 animations:^{
        [icon.layer setAffineTransform:CGAffineTransformRotate(CGAffineTransformIdentity, angle)];
        if (self->_openImage) {
            icon.image = closed ? self->_openImage : (self->_closeImage ? self->_closeImage : self->_openImage);
        }
    }];
}
...
- (void)toggleStack:(id)sender {
    if (_delegate && [_delegate respondsToSelector:@selector(statckMenuShouldOpen:)] && ![_delegate statckMenuShouldOpen:self]) {
        return;
    }
    if (!_isOpen)
        [self openStack];
...
- (void)moveItem:(UPStackMenuItem *)item toCenter:(CGPoint)center withDuration:(NSTimeInterval)duration opening:(BOOL)opening bouncing:(BOOL)bouncing {
    ...
    if (bouncing) {
        ...
        if (opening) {
            ...
            nearCenter = CGPointMake(center.x, center.y + (bouncingOffset/2)*-3);
        } else
